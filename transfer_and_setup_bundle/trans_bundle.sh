...
exec 2>logs/transfer_bundels-${version}-`date +'%Y%m%d-%H%M'`.log
set -x
}
...
if [ -f "/fsnhome/sradovic/transfer/${bundel}" ]
                        then
                                ...
                                echo "file /fsnhome/sradovic/transfer/${bundel} does not exists!!!";
