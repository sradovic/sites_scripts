...
dst_db_user=`ssh $SSHOPTS ${DEST_SITE} "grep ^db_user /bigmac/conf/database.properties | cut -f2 -d="`
dst_db_passwd=`ssh $SSHOPTS ${DEST_SITE} "grep ^db_pass /bigmac/conf/database.properties | cut -f2 -d="`
...
if echo "$g_editionName" | grep "No such file or directory" ; then 
  ssh -o StrictHostKeyChecking=no -o PasswordAuthentication=no -o LogLevel=QUIET -t -t  $dbhost_mapped  "[[ -f /usr/local/bin/oraenv ]] && source /usr/local/bin/oraenv -s <<<\"${dbsid_mapped}\" ; sqlplus $dst_db_user/$g_db_cpquserpass"
else
  ssh -o StrictHostKeyChecking=no -o PasswordAuthentication=no -o LogLevel=QUIET -t -t  $dbhost_mapped  "[[ -f /usr/local/bin/oraenv ]] && source /usr/local/bin/oraenv -s <<<\"${dbsid_mapped}\"; export ORA_EDITION="$g_editionName"; sqlplus $dst_db_user/$g_db_cpquserpass"
fi
